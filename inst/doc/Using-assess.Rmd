...
- The coefficient of determination and the adjusted coefficient of determination
  ($R^2$ and $R^2_{adj}$)
- A measure of the **effect size** ($f^2$).
...
modeled as composites, however, we explicitly warn to interpret results, 
...
### Implementation
The functions are implemented as: `calculateCFI()`, `calculateNFI()`, 
...
[^1]: There are some cutoffs such as e.g., the SRMR should be less than
      ...
      based the cutoff on a simulation using factor-based SEM.
