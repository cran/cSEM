...
See e.g., @Henseler2017 for a clarifiction.
...
1. The [Notation][Notation] files contains some fundamental mathematical notation/
   ...
   expections, we mostly follow standard notation as layed out in e.g., @Bollen1989

The package is designed based on a set of principles and are Terminology that 
are partly in contrast to commonly used open-source or commercial software packages
offering similar content (e.g., smartPLS).
Togther with the [Terminology][Terminology] and [Notation][Notation] file this introduction seeks to explains 
these principles.
...
1. **Postestimate**: Inspired by the [grammar of data manipulation](https://dplyr.tidyverse.org/) 
   ...
   as well as 4 additional test commands and 2 general do command:
   1. `assess()`
   1. `infer()`
   1. `predict()`
   1. `summarize()`
...
:   Test for group differences using several different approach such as e.g.,
...
:   Performs a floodlight analysis as described in @Spiller2013.

`doRedundancyAnalysis()`

:   Perform a redundancy analysis (RA) as proposed by @Hair2016 with reference to 
    @Chin1998.
